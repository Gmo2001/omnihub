...
class ProfileBuilder:
    ...
    def run(self, target_doc_id=None):
        try:
            Config.validate()
        ...
        if target_doc_id:
            logger.info(f"Profile Build 단일 실행: {target_doc_id}")
            doc = self.db.collection("documents").document(target_doc_id).get()
            if doc.exists:
                self.process_document(doc)
            else:
                logger.error(f"Document {target_doc_id} not found in documents")
            return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--doc_id", help="Target Document ID")
    ...
    if args.engagement_id: Config.ENGAGEMENT_ID = args.engagement_id

    builder = ProfileBuilder()
    builder.run(target_doc_id=args.doc_id)
