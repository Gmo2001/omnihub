...
class DriveSyncAgent:
    def __init__(self):
        ...
        # [Debug] Validate Bucket Name first
        logger.info(f"Initializing DriveSyncAgent... Target Bucket: '{Config.GCS_BUCKET}'")
        if not Config.GCS_BUCKET or not isinstance(Config.GCS_BUCKET, str) or not Config.GCS_BUCKET.strip():
             logger.critical(f"FATAL: Invalid GCS_BUCKET env var: '{Config.GCS_BUCKET}'")
             raise ValueError(f"Invalid GCS_BUCKET: '{Config.GCS_BUCKET}'. Check your .env or Cloud Run environment variables.")
    ...
    def fetch_all_files_recursive(self, folder_id: str, last_sync_time: Optional[str] = None, depth: int = 0) -> List[Dict]:
        """폴더 탐색 (Depth Limit 추가)"""
        if depth > 5: # Safety: 깊이 제한
            logger.warning(f"Max depth reached at folder {folder_id}")
            return []

        all_files = []
        page_token = None
        
        # 쿼리: 해당 폴더 내의 모든 파일과 폴더
            ...
            try:
                ...
                for f in items:
                    if f['mimeType'] == 'application/vnd.google-apps.folder':
                        # 폴더는 무조건 안으로 파고듭니다.
                        logger.info(f"Directory Scan (Depth {depth}): {f['name']}")
                        all_files.extend(self.fetch_all_files_recursive(f['id'], last_sync_time, depth + 1))
                    else:
                ...
                logger.error(f"Error listing files in {folder_id}: {e}")
                break
