...
const DocDetailDrawer: React.FC = () => {
    ...
    React.useEffect(() => {
        if (doc) {
            // Optional: Analytics tracking for drawer open
            // console.log("Drawer opened for", doc.id);
        }
    }, [doc]);
        ...
        <div className="absolute top-4 bottom-4 right-4 w-[420px] bg-[#09090b]/95 backdrop-blur-xl border border-white/10 shadow-2xl z-40 flex flex-col rounded-2xl overflow-hidden ring-1 ring-white/5 animate-in slide-in-from-right-4 duration-300">
            ...
            <div className="shrink-0 flex flex-col px-6 py-4 border-b border-white/10 bg-gradient-to-r from-white/5 to-transparent relative">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    {doc.ssotRating && (
                        <div className={`flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase border ${doc.ssotRating === 'gold'
            ...
            <div className="flex-1 overflow-y-auto p-6 scrollbar-thin">
                        ...
                        {!rejectMode ? (
                            <div className="grid grid-cols-2 gap-3 relative z-10">
                                <button
                                    ...
                                    className={`flex items-center justify-center gap-2 py-2.5 rounded-lg text-xs font-bold transition-all ${securityState.softBlocked
                ...
                >
                    <div className="grid grid-cols-2 gap-y-5 gap-x-4 text-xs">
                        ...
                        <div className="col-span-2 space-y-1.5">
                            ...
                            {doc.actualPath && (
                                <div className="mt-2 text-slate-400 font-mono text-[10px] break-all bg-black/20 p-2.5 rounded border border-white/5 border-dashed">
                                    <span className="text-slate-500 block mb-1 text-[8px] uppercase">Actual:</span>
                                    {doc.actualPath}
                                </div>
                            )}
                        </div>
                ...
                >
                    <div className="grid grid-cols-3 gap-3">
                        <button
                            onClick={handleOpen}
                            disabled={securityState.softBlocked}
                            className={`flex flex-col items-center justify-center p-4 rounded-xl border transition-all gap-2 group ${securityState.softBlocked
                ...
                {/* 4. Evidence Scraps (Chunks) */}
                <EvidenceSection docId={doc.id} isOpen={sections.chunks} onToggle={() => toggleSection('chunks')} />
...
// Sub-component for Chunks to handle fetching
import { OmniHubAPI } from '../services/dataService';

const EvidenceSection = ({ docId, isOpen, onToggle }: { docId: string, isOpen: boolean, onToggle: () => void }) => {
    ...
    React.useEffect(() => {
        ...
        if (isOpen && chunks.length === 0) {
            setLoading(true);
            OmniHubAPI.getDocChunks(docId).then(data => {
                if (mounted) {
    ...
    }, [docId, isOpen, chunks.length]);

    // Reset chunks when docId changes
    React.useEffect(() => {
        setChunks([]);
    }, [docId]);
    ...
    return (
        ...
        >
            {loading ? (
                <div className="text-center py-4 text-xs text-slate-500 animate-pulse">Loading chunks...</div>
            ) : chunks.length === 0 ? (
                <div className="text-center py-4 text-xs text-slate-500 italic">No structured chunks found.</div>
            ) : (
                <div className="space-y-3">
                    {chunks.map((chunk, idx) => (
                        <div key={idx} className="bg-black/20 p-3 rounded-lg border border-white/5 hover:border-indigo-500/30 transition-colors group">
                            <div className="flex justify-between items-center mb-2">
                                <span className="text-[10px] font-bold text-indigo-300 bg-indigo-500/10 px-1.5 py-0.5 rounded uppercase">Page {chunk.page}</span>
                                <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded border ${chunk.security === 'high' ? 'text-red-400 border-red-500/20 bg-red-500/10' : 'text-slate-500 border-slate-500/20 bg-slate-500/10'}`}>
                                    {chunk.security || 'LOW'}
                                </span>
                            </div>
                            ...
                            {chunk.tags && chunk.tags.length > 0 && (
                                <div className="mt-2 flex gap-1 flex-wrap">
                                    {chunk.tags.map((t: string, i: number) => (
                                        <span key={i} className="text-[9px] text-slate-400">#{t}</span>
                                    ))}
                                </div>
                            )}
                        </div>
