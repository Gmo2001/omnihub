...
const SecurityTab: React.FC = () => {
  const { 
      ...
      simulateAttack, resetSecurity, docs 
  } = useOmniHub();
                ...
                <div className="flex gap-4">
                    {securityState.mode === 'ALERT' ? (
                        ...
                        <button 
                            ...
                            className="flex items-center gap-2 px-6 py-2.5 bg-[#1E1F2E] hover:bg-red-500/10 text-red-400 border border-red-900/30 hover:border-red-500/50 rounded-xl font-bold transition-all active:scale-95"
                        >
                            <Zap size={18} />
                            SIMULATE DDOS
                        </button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[500px]">
                ...
                <div className="bg-[#1E1F2E]/60 backdrop-blur border border-white/5 rounded-2xl flex flex-col overflow-hidden">
                    <div className="px-6 py-5 border-b border-white/5 bg-white/5">
                        <h3 className="text-base font-bold text-slate-200">Live Event Feed</h3>
                    </div>
                    <div className="flex-1 overflow-y-auto p-0 scrollbar-thin">
                        <table className="w-full text-left text-xs">
                            <tbody className="divide-y divide-white/5">
                                {securityLogs.length === 0 ? (
                                    <tr><td className="px-6 py-8 text-center text-slate-600 italic">No active events.</td></tr>
                                ) : (
                                    securityLogs.map((log) => (
                                        <tr key={log.id} className="hover:bg-white/5 transition-colors group">
                                            ...
                                            <td className="px-6 py-3">
                                                <div className="text-slate-300 font-medium truncate max-w-[200px]" title={log.message}>{log.message}</div>
                                                <div className="text-[10px] text-slate-500 mt-0.5">@{log.actorRole}</div>
                                            </td>
                                            <td className="px-6 py-3 text-right">
                                                {(log.level === 'WARN' || log.level === 'ERROR') ? (
                                                     <span className="inline-flex items-center gap-1.5 text-red-400 font-bold bg-red-500/10 px-2 py-0.5 rounded text-[10px]"><Ban size={10}/> BLOCKED</span>
                                                ) : (
