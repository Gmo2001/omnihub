...
import { ConceptNode, DocRecord, Citation } from '../types';
import { ActualFolderNode, OmniHubAPI, getVirtualTreeStructure } from '../services/dataService';
import GraphVisualizer from './GraphVisualizer';
import DocDetailDrawer from './DocDetailDrawer';
import { useOmniHub } from '../context/OmniHubContext';
import { Search, Sparkles, ArrowRight, History, CloudUpload, FileSearch, Bot, Loader2, MessageSquare, FolderTree, ChevronRight, ChevronDown, Filter, Database, FileText, Star, Quote } from 'lucide-react';
...
const TreeDocItem = memo(({
    ...
    <div
        id={`tree-doc-${doc.id}`}
        onClick={() => onClick(doc)}
        className={`flex items-center gap-2 px-3 py-1.5 rounded cursor-pointer text-xs transition-all group/doc border border-transparent ${isSelected
...
const OmniHubTab: React.FC = () => {
    ...
    const [treeMode, setTreeMode] = useState<TreeMode>('actual');
    const [searchQuery, setSearchQuery] = useState('');
    const [ragResponse, setRagResponse] = useState<RagResponse | null>(null);
    const [viewModeState, setViewModeState] = useState<'default' | 'local' | 'evidence'>('default');
    const [evidenceData, setEvidenceData] = useState<{ docs: DocRecord[], concepts: ConceptNode[], citations: Citation[] } | null>(null);

    const [virtualFilters, setVirtualFilters] = useState<VirtualFilterState>({
        security: [], status: [], years: [], tags: []
    });

    const virtualTreeData = React.useMemo(() => {
        return getVirtualTreeStructure(docs, treeSearchQuery);
    ...
    const handleSearch = async (query: string) => {
        ...
        try {
            // Real Chat API Call
            const chatResult = await OmniHubAPI.chat(query);

            if (chatResult) {
                setRagResponse({
                    conclusion: chatResult.answer,
                    evidenceSummary: `핵심 증거: ${chatResult.citations.length}건의 인용구 식별됨`,
                    action: chatResult.action_item || "제안된 조치를 확인하세요."
                });

                // Also fetch Evidence Graph for Visualization
                const graphResult = await OmniHubAPI.searchEvidence(query);

                if (graphResult) {
                    // Pass backend citations to state
                    const evidenceDocs = graphResult.docs.map((d, i) => ({
                        ...d,
                        ssotRating: i === 0 ? 'gold' : (i < 3 ? 'silver' : undefined)
                    }));

                    setEvidenceData({
                        docs: evidenceDocs as DocRecord[],
                        concepts: graphResult.concepts,
                        citations: chatResult.citations || [] // Use chat citations as primary source for chips
                    });

                    setViewModeState('evidence');
                    addLog(`Evidence Graph Loaded: ${graphResult.docs.length} docs, ${chatResult.citations.length} citations`, 'INFO');
                }
            } else {
                addLog("검색/채팅 응답 실패", 'ERROR');
                setRagResponse({
                    conclusion: "AI 응답을 받아오지 못했습니다.",
                    evidenceSummary: "백엔드 연결 상태를 확인해주세요.",
                    action: "Retry later"
                });
            }
        } catch (e) {
            addLog("검색 중 오류 발생", 'ERROR');
        }
    };

    // Helper to find doc by ID
    const findDoc = (id: string) => evidenceData?.docs.find(d => d.id === id) || docs.find(d => d.id === id);
                ...
                <div className="px-4 pt-4 pb-2">
                    <div className="flex bg-white/5 p-1 rounded-xl border border-white/5">
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-bold transition-all ${leftTab === 'chat'
                        ...
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-bold transition-all ${leftTab === 'explorer'
                                ? 'bg-[#1E1F2E] text-white shadow-lg border border-white/5'
                                : 'text-slate-400 hover:text-slate-200 hover:bg-white/5'
                                }`}
                        >
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        <div className="space-y-3">
                                            <div className="bg-[#1E1F2E] border border-white/5 p-4 rounded-r-xl rounded-bl-xl text-sm text-slate-200 leading-relaxed shadow-lg">
                                                <span className="text-indigo-400 font-bold block mb-2 flex items-center gap-2">
                                                    <Sparkles size={12} /> AI Conclusion
                                                </span>
                                                {ragResponse.conclusion}

                                                {/* Citations Chips */}
                                                {evidenceData?.citations && evidenceData.citations.length > 0 && (
                                                    <div className="mt-4 flex flex-wrap gap-2 pt-3 border-t border-white/5">
                                                        {evidenceData.citations.map((cit, idx) => {
                                                            const doc = findDoc(cit.doc_id);
                                                            if (!doc) return null;
                                                            return (
                                                                <button
                                                                    key={idx}
                                                                    onClick={() => {
                                                                        handleDocSelect(doc);
                                                                        setActiveCenterId(doc.id);
                                                                        addLog(`인용구 클릭 -> ${doc.name} (p.${cit.page})`, 'INFO');
                                                                    }}
                                                                    className="flex items-center gap-1.5 px-2 py-1 rounded bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-200 text-[10px] font-bold border border-indigo-500/30 transition-all hover:scale-105"
                                                                >
                                                                    <Quote size={10} className="fill-indigo-200" />
                                                                    {idx + 1}. {doc.name.length > 15 ? doc.name.substring(0, 15) + '...' : doc.name} (p.{cit.page})
                                                                </button>
                                                            );
                                                        })}
                                                    </div>
                                                ...
                                                >
                                                    ...
                                                    <div className="flex flex-col min-w-0">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            {doc.ssotRating && <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold border ${doc.ssotRating === 'gold'
                    ...
                    <div className="flex-1 flex flex-col h-full overflow-hidden">
                        <div className="px-6 pt-6 pb-2 shrink-0">
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-2">
                                    {treeMode === 'actual' ? (
                                        <div className="flex items-center gap-2 text-amber-400 animate-in fade-in slide-in-from-left-2 duration-300">
                                            <FolderTree size={16} />
                                            <h2 className="font-bold uppercase tracking-wider text-xs">Actual File System</h2>
                                        </div>
                                    ) : (
                                        <div className="flex items-center gap-2 text-indigo-400 animate-in fade-in slide-in-from-right-2 duration-300">
                                            <Sparkles size={16} />
                                            <h2 className="font-bold uppercase tracking-wider text-xs">AI Context / Virtual</h2>
                                        </div>
                                    )}
                                </div>
                                {/* Toggle Switch */}
                                <div className="flex bg-white/5 p-0.5 rounded-lg border border-white/10">
                                    <button
                                        onClick={() => setTreeMode('actual')}
                                        className={`px-2 py-1 rounded-md text-[10px] font-bold transition-all ${treeMode === 'actual' ? 'bg-[#1E1F2E] text-amber-400 shadow' : 'text-slate-500 hover:text-slate-300'}`}
                                    >
                                        Actual
                                    </button>
                                    <button
                                        onClick={() => setTreeMode('virtual')}
                                        className={`px-2 py-1 rounded-md text-[10px] font-bold transition-all ${treeMode === 'virtual' ? 'bg-[#1E1F2E] text-indigo-400 shadow' : 'text-slate-500 hover:text-slate-300'}`}
                                    >
                                        Virtual
                                    </button>
                                </div>
                            ...
                            <div className="relative group mb-4">
                                <input
                                    ...
                                    placeholder={treeMode === 'actual' ? "Filter folders or files..." : "Filter by AI context..."}
                                    className={`w-full bg-[#050508] border text-slate-200 pl-8 pr-4 py-2 rounded-lg text-xs focus:outline-none transition-all placeholder-slate-600 ${treeMode === 'actual' ? 'border-amber-500/20 focus:border-amber-500/50' : 'border-indigo-500/20 focus:border-indigo-500/50'}`}
                                />
                        ...
                        <div className="flex-1 overflow-y-auto px-4 pb-6 scrollbar-thin">
                            <ActualTree
                                treeData={treeMode === 'actual' ? actualTreeData : virtualTreeData}
